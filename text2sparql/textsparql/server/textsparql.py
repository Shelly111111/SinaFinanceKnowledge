...
def text2sparql(request):
    query = dict(request.POST)
    query = list(query.keys())[0]
    query = jieba.cut(query,cut_all=False)
    query = [bos_id]+[get_id(vocab_dict, v) for v in query]+[eos_id]
    query = paddle.to_tensor(query)
    length = paddle.to_tensor(query.shape)
    finished_seq = model.predict_batch(inputs=[query.unsqueeze(0),length])[0]
